...
get_seq_test_comp_RCT_norm_N_unknown <- function(B,
                                                 Sigma,
                                                 side,
                                                 burnin,
                                                 m_init,
  ...
  z_agN <- qt(p = 1 - alpha * gamma / side, df = N_grid - 1)
  ...
  compare_tests <- function(m_T) {
    ...
    mu_true <- c(m_T, m)
    maxN <- max(N_grid)
    bigX <- sample_patient(maxN * B, mu_true, Sigma)
  ...
  res6 <- compare_tests(0.6)
  res8 <- compare_tests(0.8)
